...
/// Refactored UI helper methods (`_buildToolCard` and `_buildFileItem`) into dedicated `StatelessWidget`s.
/// `_FileItem` uses a `const` constructor to allow the framework to cache the list items
/// and completely skip the build phase for these sub-trees when the parent rebuilds.
/// `_ToolCard` takes a dynamic closure `onTap`, so it cannot use `const` instantiation directly,
/// but extracting it still prevents unnecessary re-creations of its internal tree compared to a helper method.
